...
- 2026-08-31 — **Day 34 generated** (headless 06:00 run). Topic: `match`/case
  ...
  **Next-day note:** with `match`/`case` now taught, the remaining
  ...
  outcome record exists for any day — no reported weak spot to target.
- 2026-09-01 — **Day 35 generated** (headless 06:00 run). Idempotency:
  confirmed before writing anything that no `python/lessons/0035-*.html` (or
  higher) file existed, `assets/nav.js` had no `n: 35`/`date: "2026-09-01"`
  entry, and `NOTES.md` had no existing `2026-09-01` entry — highest lesson
  on disk was `0034-match-case.html` dated 2026-08-31, so generation
  proceeded as Day 35, not a re-run.
  Topic selection: took Day 34's own next-day note directly — `__slots__`
  was named there as "the remaining never-covered core-language corner…a
  real Day 1/7 follow-on" and the strongest single candidate for Day 35.
  Grepped every lesson and practice file first and confirmed zero prior hits
  for `__slots__`/`slots` (the only two hits found were unrelated uses of
  the English word "slots" in Day 2/3's comprehension prose, not the
  mechanism). Did not chase `RESOURCES.md`'s still-open "Gaps" (Python
  interview prep, FastAPI project-layout reference) this run — `__slots__`
  is the more concretely-grounded, single-lesson-shaped pick, and Day 34
  already flagged the Gaps as secondary to it. Read Day 1
  (`0001-names-objects-and-mutability.html`) and Day 7
  (`0007-dataclasses-typing-capstone.html`) in full before drafting, so
  today's lesson explicitly builds on both: section 1 reuses Day 1's
  names-are-labels/objects-live-in-memory picture to introduce the
  per-instance `__dict__` every plain object carries, and section 5 lands on
  Day 7's `@dataclass` directly via `slots=True`.
  Taught: section 1 the problem live (a plain instance's attributes live in
  a hidden per-instance `__dict__`; any name can be attached to any
  instance, so a typo like `p.yz = 5` silently succeeds instead of raising);
  section 2 `__slots__` as a tuple of allowed attribute names, removing the
  dict entirely — declared attributes work exactly as before, undeclared
  ones now raise `AttributeError` immediately; section 3 the memory case,
  with numbers measured live rather than asserted; section 4 the
  inheritance gotcha (a subclass without its own `__slots__` silently
  regains a `__dict__`) and the fix (each class in the chain declares only
  the fields it itself adds); section 5 `@dataclass(slots=True)`, confirming
  `__repr__`/`__eq__` are unaffected. Bridged from SQL per the baseline
  record: a table with a fixed set of typed columns doesn't let a row
  silently grow an undeclared column, introduced before any Python in the
  top callout, not from a pandas analogy.
  **Interpreter-behavior claims verified live, not just from docs**, per Day
  34's own corrected-mistake precedent — all run via scratch `uv run
  python3` probes against this course's actual Python (confirmed
  `3.12.3`, matching Day 34's interpreter) before being stated as fact:
  (1) a plain instance has `__dict__` and accepts a dynamic attribute; a
  `__slots__` instance has no `__dict__` (`AttributeError: 'Slotted' object
  has no attribute '__dict__'`) and rejects an undeclared attribute,
  including a deliberate typo case (`AttributeError: 'Slotted' object has
  no attribute 'yy'`) — all three outcomes matched what was drafted, no
  correction needed;
  (2) `dataclass(slots=True)` runs cleanly on 3.12.3 with no error, and a
  slotted dataclass instance's `__repr__`/`__eq__` behave identically to the
  non-slotted version — also matched the draft;
  (3) the inheritance case was checked specifically because it was the
  least obvious claim: a `Child(Base)` subclass that declares no `__slots__`
  of its own regains a real `__dict__` and accepts an arbitrary dynamic
  attribute (`cn.z = 99` succeeded, `hasattr(cn, "__dict__")` was `True`),
  even though `Base` itself declared `__slots__` — confirmed independently
  against `docs.python.org/3/reference/datamodel.html#slots` via `WebFetch`
  (succeeded), which states a subclass without its own `__slots__` gets a
  `__dict__` back because the base's absence of a *class-level* guarantee
  doesn't propagate; the two sources agreed, so no correction was needed
  here either, but this is the specific line this run treated as
  highest-risk and checked two ways before writing section 4;
  (4) the exact per-instance memory-saving numbers in section 3 (136.0
  bytes/instance plain vs. 48.0 bytes/instance slotted, ~2.83x, over
  100,000 instances of a two-field class, via `sys.getsizeof`) were
  generated by the scratch probe itself and pasted directly into the
  lesson, not estimated;
  (5) `WebFetch` against `docs.python.org/3/library/dataclasses.html`
  (succeeded) confirmed `slots=True` was added in Python 3.10 specifically,
  used as the primary source for that version claim in section 5 rather
  than assumed. No claim in the shipped lesson required a correction this
  run — unlike Day 34, every drafted behavioral claim matched its live
  probe on the first attempt.
  No-pandas rule: zero pandas/NumPy/`pd.`/`np.` hits in the practice file
  (grepped case-insensitively — zero hits, only stdlib `from dataclasses
  import dataclass`); exactly one hit in the lesson — one contrast
  sentence/callout (titled "Where pandas goes from here") naming that a
  DataFrame already gets `__slots__`'s memory win for free via a different,
  columnar mechanism, without demonstrating any pandas API, placed once
  after section 5/the interview callout, matching the hard-rule section
  above.
  Practice file `practice/35_slots.py` (4 exercises: adding `__slots__` to
  a plain `Point` class and confirming both the declared-field-still-works
  and undeclared-attribute-raises outcomes, fixing a `Child(Base)`
  inheritance chain so the subclass doesn't silently regain a `__dict__`,
  converting a Day-7-style `@dataclass` to `slots=True` while confirming
  `__eq__`/`__repr__` still work, and writing a small `has_instance_dict()`
  helper) needed no on-disk fixtures. Followed this course's standard
  defensive pattern against the Ellipsis-at-module-level bug family:
  Exercises 1-3's TODOs are comment-only markers next to already-valid class
  bodies (`Point`'s body already has a real `__init__`; `Child`'s body has
  `pass`, a legal statement; `Record`'s decorator line has no placeholder at
  all) — none of them need a bare `...` at class or module scope, sidestepping
  the bug family entirely rather than relying on discipline; only Exercise
  4's TODO uses `...`, and it lives strictly inside `has_instance_dict()`'s
  function body. Verified in a scratch dir under the repo root
  (`.scratch/python-lesson35/`, removed after use — direct `/tmp` writes
  remain sandbox-blocked per every prior day's precedent, reconfirmed this
  run by using the repo-root scratch dir from the start): the shipped
  (unsolved) copy, run via plain `uv run python3` (no `--with` needed), both
  from the scratch copy and from its real `practice/` path with the
  documented command, printed four clean ✗ lines with no traceback each
  time; a separately solved copy (every `...`/TODO filled in by hand in the
  scratch copy, not assumed) printed all four ✓ and the "All green" tally.
  No bugs found during verification — both passes succeeded on the first
  attempt.
  Glossary: added a Day 35 section to `reference/glossary.html`
  (`__slots__`, `instance __dict__`) after confirming via grep that neither
  term collided with any Day 1-34 entry. The 2 new `<dfn data-en` tags in
  the lesson body match the 2 new glossary rows exactly (confirmed by
  count).
  Quiz: 5 questions. Word counts were checked with a small Python script
  (regex-splitting the quiz block into its five question `<div>`s and
  counting `.split()` words per `<button class="opt">` line, run via `uv run
  python3`) and mismatched on the first draft for four of five questions
  (Q1 7/9/7, Q2 8/9/8, Q4 7/10/9, Q5 7/10/8; Q3 was 8/8/8 correct
  immediately). Each mismatched question went through two to three
  rewrite-and-recount rounds — several rewrites landed on plausible-looking
  option text that still miscounted on the next run, a reminder that
  eyeballing word count by reading the sentence is unreliable even for a
  generator — re-running the script after every edit, until every option
  matched (Q1 8/8/8, Q2 9/9/9, Q3 8/8/8, Q4 9/9/9, Q5 9/9/9). Re-verified
  with a second, independently-written script (document-order `<button
  class="opt">` extraction via regex across the whole file, ignoring the
  per-question `<div>` wrapper entirely, then grouped in threes) run against
  the final file — independently confirmed all 15 options (5 questions × 3
  options) land on their target counts before shipping. HTML tag-balance
  also checked with a small script (counting open vs. close tags for every
  non-void element across the file) — all tags balanced on first check, no
  fixes needed. Registered in `assets/nav.js` with `date: "2026-09-01"`.
  **DB access:** per this run's own instructions, both `bin/query-progress`
  and `bin/record-progress` were attempted once each — both were blocked
  before executing ("This command requires approval"), the same sandbox
  gate documented on nearly every prior day since Day 8. Not retried. This
  run paced entirely from on-disk state (`NOTES.md`'s own generation log,
  `python/lessons/`, `python/assets/nav.js`, `python/learning-records/` —
  still only the Day 1 baseline, no completion/quiz/kata outcome record for
  any day 2-34).
  **Next-day note:** with `__slots__` now taught, `PLAN.md`'s spine and
  every previously-named live candidate (`enum`, `match`/`case`,
  `__slots__`) are exhausted. Day 36 has no single named-but-untaught
  mechanism left from any prior day's own notes — the strongest concretely-
  grounded next step is finally attempting `RESOURCES.md`'s two still-open
  "Gaps" (a Python interview-prep source, a FastAPI project-layout
  reference) via `WebSearch`, both flagged as open since Day 25/28 and
  deliberately deferred by Days 33/34/35 in favor of clearer single-lesson
  picks that no longer exist. Failing that, a testing-focused retrieval day
  (Day 11's `pytest` fixtures/parametrize combined with Day 22's
