...
- 2026-08-30 (headless 06:00 run): skipped again per the same rule — the
  ...
  example — `day:` tags still run 1-7 only, `nav.js` `LESSONS` still
  registers only the 7 Jul 8-14 lessons).
- 2026-08-31 (headless 06:00 run): skipped again per the same rule — the
  course window closed 2026-07-20, now forty-two days past it. Day 13
  (`0014-concurrency-capstone.html`) re-confirmed present on disk and
  registered in `assets/nav.js` — moot either way. This session's sandbox
  again hard-blocked any command referencing `LEARNING_DB_URL` directly
  ("Contains simple_expansion"). This round, the `bin/record-progress`
  write path — which had reliably worked in prior rounds when invoked
  directly with a literal command — also hit the generic "requires
  approval" gate with no user present, both for the orchestrator and for
  each delegated agent (backend, data, python all reported the same
  failure independently). Moot for this course since it's a pure skip
  regardless of DB state. Backend lesson 57 (cache stampede / thundering
  herd), data lesson 54 (`.equals()`/`.compare()`), and python Day 34
  (`match`/`case` structural pattern matching) were generated this round
  via delegated agents, each confirmed to have written only within its own
  course directory — see their own NOTES.md entries for detail; rust's
  post-week daily quiz+kata infra was re-verified unchanged (7 real kata
  entries `k1`-`k7`, `day:` tags still run 1-7 only, `nav.js` `LESSONS`
  still registers only the 7 Jul 8-14 lessons).
- 2026-09-02 (headless 06:00 run): skipped again per the same rule — the
  course window closed 2026-07-20, now forty-eight days past it. All 14
  ...
  and every helper script wrapping it (`bin/query-progress`) were blocked
  again this session ("Contains simple_expansion" content-level block on
  the raw variable reference, plus a generic "requires approval" gate on
  the helper script with no user present) — same two classes of block as
  every prior round — so no `course_progress` rows could be read; fell
  back to file-state + learning-records alone per the standing fallback
  rule. Moot for this course since it's a pure skip regardless of DB
  state. Backend lesson 59 (row-level security), data lesson 56
  (`json_normalize()`), and python Day 36 (`functools`) were generated
...
- 2026-09-05 (headless 06:00 run): skipped again per the same rule — the
  ...
  lessons).
- 2026-09-06 (headless 06:00 run): skipped again per the same rule — the
  course window closed 2026-07-20, now fifty-two days past it. All 14
  lessons (`0001`-`0014`) re-confirmed present on disk and registered in
  `assets/nav.js` — moot either way. `bin/query-progress` and direct `psql
  "$LEARNING_DB_URL" ...` were both blocked again this session (a generic
  "requires approval" gate on the former with no user present, "Contains
  simple_expansion" content-level block on the latter, reproduced even
  inside a plain `for` loop over an unrelated shell variable) — same two
  classes of block as every prior round — so no `course_progress` rows
  could be read; fell back to file-state + learning-records alone per the
  standing fallback rule. Moot for this course since it's a pure skip
  regardless of DB state. Backend lesson 63 (polling/long-polling/SSE/
  WebSockets), data lesson 60 (`str.contains()` and the `na=` trap), and
  python Day 40 (`contextlib.suppress()`/`ExitStack`) were generated this
  ...
  rust's post-week daily quiz+kata infra was re-verified unchanged (8
  textual `id: "k` matches — 7 real kata entries `k1`-`k7` plus the
  commented-out example line — `day:` tags still running 1-7 only,
  `nav.js` `LESSONS` still registers only the 7 Jul 8-14 lessons).
- 2026-09-07 (headless 06:00 run): skipped again per the same rule — the
  course window closed 2026-07-20, now fifty-three days past it. All 14
  lessons (`0001`-`0014`) re-confirmed present on disk and registered in
  `assets/nav.js` — moot either way. `bin/query-progress` hit the usual
  generic "requires approval" gate with no user present, and direct `psql
  "$LEARNING_DB_URL" ...` was blocked again this session ("Contains
  simple_expansion" content-level block on the raw variable reference) —
  same two classes of block as every prior round — so no `course_progress`
  rows could be read; fell back to file-state + learning-records alone per
  the standing fallback rule. Moot for this course since it's a pure skip
  regardless of DB state. Backend lesson 64 (secrets management), data
  lesson 61 (`groupby().apply()` and multi-function `.agg()`), and python
  Day 41 (`typing` extras: `Protocol`/`TypedDict`/generics) were generated
  this round via delegated agents, each confirmed to have written only
  within its own course directory; see their own NOTES.md entries for
  detail; rust's post-week daily quiz+kata infra was re-verified unchanged
  (8 textual `id: "k` matches — 7 real kata entries `k1`-`k7` plus the
  commented-out example line — `day:` tags still running 1-7 only,
  `nav.js` `LESSONS` still registers only the 7 Jul 8-14 lessons).
- 2026-09-09 (headless 06:00 run): skipped again per the same rule — the
  course window closed 2026-07-20, now fifty-one days past it. All 14
  lessons (`0001`-`0014`) re-confirmed present on disk and registered in
  `assets/nav.js` — moot either way. `bin/query-progress` hit the usual
  generic "requires approval" gate with no user present, and direct `psql
  ...
  rows could be read; fell back to file-state + learning-records alone per
  the standing fallback rule. Moot for this course since it's a pure skip
  regardless of DB state. Backend lesson 66 (naming the API gateway/BFF
  edge layer that lessons 11, 28, 31, and 65 had assumed but never
  located), data lesson 63 (`interpolate()`, the fourth missing-data fill
  strategy), and python Day 43 (a cross-cutting review/retrieval day over
  Days 1, 2, 3, 5, 7, and 12, chosen because zero completion/quiz signal
  has ever been recorded for that course) were generated this round via
  delegated agents, each confirmed to have written only within its own
  course directory; see their own NOTES.md entries for detail; rust's
  post-week daily quiz+kata infra was re-verified unchanged (8 textual
