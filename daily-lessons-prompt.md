...
5. **`python/`** — one short (~20 min) lesson per day, started 2026-07-29 (the Python language itself: data handling and pipelines, then a FastAPI backend). **Phase 1 (Days 1–7, 2026-07-29 → 2026-08-04) is date-locked** to the filenames pre-assigned in `python/PLAN.md` — use them exactly and never pre-generate ahead of the date, same as the Go course. From 2026-08-05 it becomes open-ended and sequential from `0008-…`, following PLAN.md's Phase 2 spine, with the same idempotency rule as backend/data. Day 1 was written by hand on 2026-07-29, so the first lesson generated here is Day 2. Practice files are **standard library only** — run them with `uv run python3 …` (no `--with`), and verify the shipped form prints ✗s without a traceback and a solved copy prints all ✓, in a scratch dir.
   - **Hard scope rule:** never teach a pandas or NumPy API in this course — that is `data/`'s job and duplicating it is forbidden by `python/MISSION.md`. A one-line contrast is fine; a worked pandas example is not. When a topic could fit either course, ask whether the subject is the *language* (here) or the *library* (`data/`).

6. **`dataeng/`** — dbt, Kafka & Airflow, built as a portfolio pipeline in the learner's own public repo (`food-delivery-pipeline`, outside this workspace). **Phase 1 (Days 1–7, 2026-09-15 → 2026-09-21) is date-locked** to the filenames pre-assigned in `dataeng/PLAN.md`: use them exactly and never generate ahead of the date. Before 2026-09-15, skip this course ("not started"). From 2026-09-22 it becomes open-ended and sequential from `0008-…` along PLAN.md's Phase 2 spine, with the same idempotency rule as backend/data. There is no `practice/` directory: each lesson ends with a build step in the learner's repo plus a Verify command with expected output. Verify dbt snippets with `uv run --with "dbt-postgres==1.11.0" dbt parse --profiles-dir .` in a scratch dir (no DB needed; grep the output for `Error`, since the exit code is unreliable through a pipe) and Python files with `uv run python3 -m py_compile`, per `dataeng/NOTES.md`.
   - **Hard scope rule:** no pandas (that's `data/`), don't teach Python language features (that's `python/`), don't re-derive API/idempotency concepts (that's `backend/`). Use all three as one-line bridges only. dbt goes deep; Kafka and Airflow stay at the working level the portfolio needs.
...
3. Otherwise generate today's lesson following that workspace's `NOTES.md` conventions **exactly** — they are authoritative over anything in this prompt. In particular (all courses share these):
...
- Finish with a one-line summary per course, EXACTLY this shape in plain text (no backticks, no bold, no bullet list — automation greps this line): `go: generated 000N-… | rust: skipped (exists) | backend: … | data: … | python: … | dataeng: …`.
